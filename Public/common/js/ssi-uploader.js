...
//----------------------------UPLOADFILES------------------------------------
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                setElementMessage(thisS, ii, dataType, msg, title,returnMsg);
    ...
    var setElementMessage = function (thisS, index, msgType, msg, title,returnMsg) {
        ...
        element.after(getResultMessage(msgType, msg, title, className,returnMsg))
         .remove();
    };
    ...
    var getResultMessage = function (type, msg, title, classes,returnMsg) {//return a message label
    ...
    var locale = {
		...
		zh: {
            ...
            clear: '清除',
            drag: '将文件拖拉进框中',
            sizeError: '$1 超过了最大上传尺寸 $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 文件类型不支持',//$1=file extension ie(exe files are not supported)
