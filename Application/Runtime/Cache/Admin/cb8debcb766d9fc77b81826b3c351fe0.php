    ...
    <div class="xs">
        ...
        <div style="background: #f5f5f5;width: 100%;height: 50px">
            <ol class="breadcrumb" style="float: left;width: 20%">
                <a class="btn btn-default" href="<?php echo U('Wchat/keywords_add');?>" role="button">新增关键字</a>
            </ol>
        ...
        <div class="bs-example4" data-example-id="contextual-table">
            <table class="table">
                ...
                <?php if(is_array($keywords_data)): $i = 0; $__LIST__ = $keywords_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="active">
                    <td><input type="checkbox" name="id[]" value="<?php echo ($v["id"]); ?>" class="check_all"></td>
                    <td><?php echo ($v["name"]); ?></td>
                    <td><?php echo ($v["command"]); ?></td>
                    <td><?php echo ($v["extend"]); ?></td>
                    <td><?php if($v['type'] == 1): ?>自定义文字<?php else: ?>自定义图文<?php endif; ?></td>
                    <td><?php echo ($v["demand"]); ?></td>
                    <td>
                        <a class="btn btn-xs btn-info" href="<?php echo U('Wchat/keywords_edit/',array('id'=>$v['id'],'nav_id'=>$_GET['nav_id']));?>" role="button">修改</a>
    ...
    $(function () {
        ...
        $('.delete').click(function () {
            ...
            layer.confirm('您确定要删除此关键字吗？', {
                ...
                $.post("<?php echo U('Wchat/keywords_delete');?>",{id:id},function (result) {
